...
- `src/lib/components/doc/`: `DocShell` (full-width 23.75rem left sidebar / centered `max-w-3xl` content / 23.75rem graph+TOC sidebar, mobile slide-in at ≤800px), `Sidebar`, `NavTree` (native `<details>`, natural sort), `Toc`, `DocPager`, `enhancements.ts` (drop `filePreviews`, `kotlinPlayground`). Both desktop rails use a solid `bg-bg` panel and a 1px boundary against the reading workspace; the right rail follows Postframe's compact inspector pattern with divided graph and outline sections.
